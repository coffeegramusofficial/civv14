...
// Stock decks for each faction
const STOCK_DECKS: Record<Faction, string[]> = {
  whites: [
    "Юнкер",
    "Пулеметчик «Максим»",
    "Козак",
...
export const initializeStockDecks = (): void => {
  const decks = getAllDecks();
  
  // Check if stock decks already exist for both factions
  const hasWhiteStock = decks.some(d => d.name === "Active_whites");
  const hasRedStock = decks.some(d => d.name === "Active_reds");
  
  // Initialize White Army stock deck if it doesn't exist
  if (!hasWhiteStock) {
    const whiteCards = STOCK_DECKS.whites
      .map(name => CARDS.find(c => c.name === name && c.faction === "whites"))
      .filter((c): c is CardData => c !== undefined);
    
    if (whiteCards.length === 8) {
      saveDeck("Active_whites", "whites", whiteCards);
    }
  }
  
  // Initialize Red Army stock deck if it doesn't exist
  if (!hasRedStock) {
    const redCards = STOCK_DECKS.reds
      .map(name => CARDS.find(c => c.name === name && c.faction === "reds"))
      .filter((c): c is CardData => c !== undefined);
    
    if (redCards.length === 8) {
      saveDeck("Active_reds", "reds", redCards);
    }
  }
};
